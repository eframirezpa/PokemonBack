...
const confirm = async (req, res, next) => {
  try {
    ...
    if (r.error === 'boon')           return res.status(400).json({ error: 'Debes elegir un rasgo' })
    if (r.error === 'pathstat')       return res.status(400).json({ error: 'Debes elegir una característica' })
